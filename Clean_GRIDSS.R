...
vcf <- readVcf(paste("./seqs/4_GRIDSS/", chromosome, "/", chromosome, ".vcf", sep=""))
...
filter_BD <- function(BD, coords){
  ...
  BD_filtered <- BD_filtered %>% select(-c("chrom2", "name"))  
...
write.table(INV_data, paste("./seqs/5_Clean_GRIDSS/", strain, "_SVclean_", chromosome, ".txt", sep=""))
write.table(bedpe, paste("./seqs/5_Clean_GRIDSS/", strain, "_SVall_", chromosome, ".txt", sep=""))